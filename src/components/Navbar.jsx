...
const Navbar = ({ onRegisterHotel }) => {
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full flex items-center justify-between px-4 md:px-8 lg:px-10 xl:px-15 transition-all duration-500 z-50 ${isScrolled ? "bg-white/80 shadow-md text-gray-700 backdrop-blur-lg py-3 md:py-4" : "py-4 md:py-6"}`}>
      {/* Logo */}
      <Link to="/">
      ...
      <div className="hidden md:flex items-center gap-4 lg:gap-8">
        ...
        {/* Dashboard — only for hotel owners */}
        {user && isOwner && (
          <button
        ...
        {/* Register Hotel — only for logged-in non-owners */}
        {user && !isOwner && (
          <button
        ...
        {user ? (
          <UserButton>
            <UserButton.MenuItems>
              ...
              {isOwner && (
                <UserButton.Action label="Dashboard" labelIcon={BookIcon} onClick={() => navigate("/owner")} />
              )}
            </UserButton.MenuItems>
          </UserButton>
        ) : (
      ...
      <div className={`fixed top-0 left-0 w-full h-screen bg-white text-base flex flex-col md:hidden items-center justify-center gap-6 font-medium text-gray-800 transition-all duration-500 ${isMenuOpen ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        {navLinks.map((link, i) => (
          <Link key={i} to={link.path} onClick={() => setIsMenuOpen(false)}>{link.name}</Link>
        ))}

        {/* Mobile: Dashboard for owners, List Hotel for non-owners */}
        {user && isOwner && (
          <button
        ...
        {user && !isOwner && (
          <button
            className="border px-4 py-1 text-sm font-light rounded-full cursor-pointer transition-all"
            onClick={() => { onRegisterHotel?.(); setIsMenuOpen(false); }}
          >
            List Your Hotel
          </button>
        )}
